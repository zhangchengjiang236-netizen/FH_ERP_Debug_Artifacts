...
source_base=https://raw.githubusercontent.com/zhangchengjiang236-netizen/FH_ERP_Debug_Artifacts/420b42bef9b71187aced8ead0d699992906bee58/debug-ota-rehearsal
update=feihong-erp-0.9.7-debug-update.apk
update_hash=2dc392da270ab76214666aacca7233cc33a088977458a214d0d05312bfd5e93f
update_size=1027252
manifest_hash=bd97ec726c17bf8cb3e55bb546a6dc46a3ef2dc876fae13e947c7c9c47d0133d
ts=$(date -u +%Y%m%dT%H%M%SZ)
backup_root=/var/backups/nginx/erp-uat-debug-ota-0.9.7-$ts
tmp=$(mktemp -d /tmp/erp-debug-ota-0.9.7.XXXXXX)
finished=0
...
main() {
  ...
  python3 -c "import json,sys; d=json.load(open(sys.argv[1],encoding='utf-8')); e={'versionName':'0.9.7-debug.1','versionCode':90701,'minimumVersion':'90601','mandatory':False,'downloadUrl':'https://api-test.scxmj.cn/mobile-updates/debug-packages/feihong-erp-0.9.7-debug-update.apk','fileSize':1027252,'sha256':'2dc392da270ab76214666aacca7233cc33a088977458a214d0d05312bfd5e93f','signingIdentity':'sha256:a182e8f2b6b60e71acf6e29184558fd3b210b5c8748a8beeb75de0bace3355a4','channel':'debug'}; assert all(d.get(k)==v for k,v in e.items()); assert isinstance(d.get('publishedAt'),str) and d['publishedAt']; assert isinstance(d.get('releaseNotes'),str) and d['releaseNotes']" "$tmp/debug.json"
  ...
  if [ -f "$root/android/debug.json" ]; then
    ...
    cp -a "$root/android/debug.json" "$root/archive/debug.json.$ts.before-0.9.7"
  fi
  ...
  trap - ERR INT TERM
  rm -rf "$tmp"
  printf 'DEBUG_OTA_0_9_7_STATIC_DEPLOYED\nMANIFEST_URL=%s\nMANIFEST_SHA256=%s\nUPDATE_URL=%s\nUPDATE_SHA256=%s\nUPDATE_SIZE=%s\nSIGNER_SHA256=%s\nSTABLE_JSON=%s\nBACKUP_ID=%s\nSTATUS=PENDING_REAL_DEVICE_DEBUG_OTA_VERIFICATION\n' \
    "$manifest_url" "$manifest_hash" "$update_url" "$update_hash" "$update_size" \
    a182e8f2b6b60e71acf6e29184558fd3b210b5c8748a8beeb75de0bace3355a4 "$stable_after" "erp-uat-debug-ota-0.9.7-$ts"
}
